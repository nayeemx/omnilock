...
$env:TAURI_SIGNING_PRIVATE_KEY="<key content from src-tauri\update.key>"
$env:TAURI_SIGNING_PRIVATE_KEY_PASSWORD="229689"
npx tauri build
```

Build output: `src-tauri/target/release/bundle/` (MSI, NSIS exe, .sig signatures)
Portable exe: `src-tauri/target/release/omnilock.exe`
...
| 2FA setup/enable/disable | ✅ Real | ✅ Base32 secret encoding | Compatible with all major authenticator apps |
...
| Auto-update (GitHub Releases) | ✅ Real | ✅ Signed builds | Check for updates in Security page |
...
- **Last change:** 2FA base32 fix, auto-update feature, signing key generation
- **Key files modified this session:** totp.rs, SecurityPage.tsx, SetupWizard.tsx, lib.rs, tauri.conf.json, tauri-bridge.ts

## Signing Key (for auto-updates)
- **Private key:** `src-tauri/update.key` (keep secret!)
- **Public key:** `src-tauri/update.key.pub`
- **Password:** `229689`
- **Environment variables for signed builds:**
  ```
  TAURI_SIGNING_PRIVATE_KEY=<contents of update.key>
  TAURI_SIGNING_PRIVATE_KEY_PASSWORD=229689
  ```
...
13. **DO NOT** use base64 encoding for TOTP secrets — authenticator apps require base32 (RFC 4648)
...
7. **Auto-update not working:** Check that `TAURI_SIGNING_PRIVATE_KEY` env var is set during build. Verify GitHub repo `nayeemx/omnilock` has a release with assets.
